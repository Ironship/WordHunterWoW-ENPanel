...
local function hookTooltipsClassic()
  ...
  for _, tooltip in ipairs(tooltips) do
    if type(tooltip) == "table" and type(tooltip.HookScript) == "function" then
      hookedAny = true
      -- These are Blizzard's own frames, so they carry no argument shim: 1.12
      -- hands a handler nothing and the tooltip has to come from the closure.
      -- The script names themselves may also not exist on this client, so each
      -- hook is attempted rather than assumed.
      local tt = tooltip
      local function hook(script, handler)
        pcall(tt.HookScript, tt, script, handler)
      end
      hook("OnTooltipSetItem", function()
        if type(tt.GetItem) ~= "function" then return end
        local _, link = tt:GetItem()
        if not usable(link) or type(link) ~= "string" then return end
        local id = tonumber(string.match(link, "item:(%d+)"))
        if id then appendName(tt, "item", id) end
      end)
      hook("OnTooltipSetSpell", function()
        if type(tt.GetSpell) ~= "function" then return end
        -- Classic returns name, rank, id. Retail's script path (if it ever
        -- lands here) returns name, id. Take the last number.
        local a, b, c = tt:GetSpell()
        local id = type(c) == "number" and c or b
        if usable(id) and type(id) == "number" then appendName(tt, "spell", id) end
      end)
      hook("OnTooltipSetUnit", function()
        -- 1.12 has no GameTooltip:GetUnit, and neither does ClassicAPI -- it
        -- adds HasUnit and GetUnitGUID in its place. Asking for the unit token
        -- was an error on every creature the player pointed at.
        local guid
        if type(tt.GetUnitGUID) == "function" then guid = tt:GetUnitGUID() end
        if not usable(guid) and type(UnitGUID) == "function" then
          guid = UnitGUID("mouseover")
        end
        if usable(guid) then appendName(tt, "npc", npcIdFromGuid(guid)) end
      end)
    end
  end
  return hookedAny
...
local function hookTooltips()
  -- An English client has nothing for this feature to add, which appendName
  -- has always known -- but it only found out after the hook had already run
  -- and read the tooltip. Deciding here instead means the tooltips are left
  -- alone entirely: no call on every frame the player points at, and no way
  -- for a missing accessor to raise on a client where the answer would have
  -- been "nothing to say" anyway.
  if clientIsEnglish() then return false end
